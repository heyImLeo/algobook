...
- Protected routes live under `src/routes/_auth/**`, enforced by `beforeLoad` in `src/routes/_auth/route.tsx`.
- Guest-only routes live under `src/routes/_guest/**`, enforced by `beforeLoad` in `src/routes/_guest/route.tsx`.
